...
const HumanFigure = ({ variant = "will" }: { variant?: "henry" | "will" }) => {
  const tint = variant === "henry" ? "hsl(0 0% 8%)" : "hsl(220 30% 8%)";
  return (
    <svg viewBox="0 0 200 400" className="h-full w-auto opacity-40 breathe" preserveAspectRatio="xMidYMax meet">
      <defs>
      ...
      {/* eyes glow (subtle) */}
      <circle cx="92" cy="58" r="1.2" fill="hsl(0 100% 60%)" opacity="0.6" />
      <circle cx="108" cy="58" r="1.2" fill="hsl(0 100% 60%)" opacity="0.6" />
    </svg>
  );
};
